...
// /ingest/* 已挂权限校验（query 档读、audit 档写），必须带登录 token；
// 此前这里用裸 axios 不带 token —— 加鉴权后这些请求会全部 401。
...
async function handleFiles(rawFiles) {
  ...
  for (const f of kept) {
    ...
    fd.append('file', f)
    try {
      const res = await http.post('/ingest/upload', fd)
...
async function loadFormats() {
  try {
    const res = await http.get('/ingest/formats')
...
async function loadList() {
  ...
  try {
    ...
    const res = await http.get('/ingest/list', { params })
...
async function handleReingest(row) {
  try {
    await http.post(`/ingest/${encodeURIComponent(row.doc_id)}/reingest`)
...
async function handleDelete(row) {
  try {
    ...
    const res = await http.delete(`/ingest/${encodeURIComponent(row.doc_id)}`)
