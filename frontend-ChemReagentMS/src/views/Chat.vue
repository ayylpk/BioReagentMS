...
// ⚠️ 生成的 HTML 必须过一趟 DOMPurify 再交给 v-html。
//    这里渲染的是模型输出，而模型会把知识库原文照抄进回答 ——
//    知识库文档是用户上传的（.html / SDS 原文都可能），里面塞一段 <img src=x onerror=...>
//    就会在本页执行脚本；而登录凭证存在 localStorage，脚本可以直接读走（= 会话被接管）。
//    marked 只负责"markdown → HTML"，它不做任何安全过滤。
function md(text) {
  if (!text) return ''
  return DOMPurify.sanitize(marked(text, { breaks: true }))
}

// 在途流式请求的控制器：切走页面/组件卸载时中止它。不中止有两个后果 ——
//   ① fetch 还在读、回调还在改已卸载组件的状态；
//   ② 服务端会把这一个回合跑完（继续调用模型、继续消耗额度），而用户完全看不见。
let abortCtl = null
onUnmounted(() => {
  abortCtl?.abort()
  abortCtl = null
...
async function send() {
  ...
  try {
    ...
    // 带上登录 token：服务端（tsAgent/src/service/routes/stream.ts）按 token 判定内部/演示通道。
    // 不再上传 mode —— 权限档位由客户端自报等于可绕过：不传 mode 时服务端会当成内部通道，
    // allowDb 放开且完全不过限流，匿名即可查内部库存台账。
    ...
    // 主动中止（切页/卸载触发的 abort）不是错误：既不提示，也不要往气泡里写"请求失败"
...
<template>
  <div class="chat-container" :class="{ public: publicMode }">
    <div v-if="publicMode" class="public-banner">
      <span class="pb-title">ChemReagentMS · 实验室助手免登录体验</span>
