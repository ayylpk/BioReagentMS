...
// ---------- 分类列表（下拉用） ----------
// 说明：后端目前只有 reagent.category_id，**没有分类字典表、也没有分类接口**，
// 所以下面这份映射就是当前唯一的口径。
// 之前它被写成 async + try/catch 的样子，看着像在调接口、实际是写死的数组（catch 体还是空的）——
// 会让人误以为"后端加分类这里就会自动跟着变"。现在如实写成常量，等后端出字典接口再换成请求。
...
function loadCategories() {
  categories.value = CATEGORY_OPTIONS
}
