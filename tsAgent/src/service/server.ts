// 周边 HTTP 服务（9/6 起 = agent 侧唯一常驻进程）：摄取 API + 两条人审 + 缺口知识 + 聊天流式端点
// :8123 承接 vite 代理：/ingest、/agent→runs/stream、/review、/reaction、/gap
//   （自写线协议最小子集，替代 langgraph dev :2024）
import { serve } from '@hono/node-server'
import { Hono } from 'hono'
import { config } from '../config/env'
import { reviewRoutes } from './routes/review'
import { reactionReviewRoutes } from './routes/reactionReview'
...
app.route('/reaction', reactionReviewRoutes) // 禁配规则人审（A 线，DDL: deploy/sql/02）
