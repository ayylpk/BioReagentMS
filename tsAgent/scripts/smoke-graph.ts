// 四路冒烟：bun run scripts/smoke-graph.ts
// 一条命令验证 router→{db|knowledge|reaction|chat}→(缺口问答)→result 全链，打印路由判定+回答头一段
...
const CASES: { q: string; expect: string }[] = [
	...
	{ q: '硫酸能不能和高锰酸钾放在一起？', expect: 'reaction' }, // 禁配 → 已审核规则库
